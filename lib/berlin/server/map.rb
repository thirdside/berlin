class Berlin::Server::Map < ::Map
  ...
  def build
    ...
    parsed['types'].each do |type|
      @types[type['name']] = Berlin::Server::NodeType.parse( type )
    end

    # build each node
    parsed['nodes'].each do |node|
      @nodes << Berlin::Server::Node.parse( self, node )
    end

    # build paths
    parsed['paths'].each do |path|
      @paths << Berlin::Server::Path.parse( path )
